...
Contains
  Subroutine solve()
    !Include 'link_fnl_shared.h'  !// 安装imsl函数库的可以调用求解逆矩阵, 去掉13，14，46，54，55行的注释，并注释56行即可
    !use LINRG_INT
    Implicit none
    Integer :: i, j
    Real(kind=8) :: h, k, x, t, sigma  !// D为扩散系数，h为空间步长，k为时间步长
    Real(kind=8) :: a(numH-1,numH-1), u(0:numH,0:numT), Lside(0:numT), Rside(0:numT)
    Real(kind=8) :: tmp_u(numH-1,1), b(numH-1,1), slu(numH-1,1), Inva(numH-1,numH-1)
    ...
    Do j = 0, numT-1
      ...
      b(numH-1,1) = Rside(j)
      b(2:numH-2,1) = 0.d0
      tmp_u(1:numH-1,1) = u(1:numH-1,j) + sigma * b(1:numH-1,1)
      slu = 0.d0
